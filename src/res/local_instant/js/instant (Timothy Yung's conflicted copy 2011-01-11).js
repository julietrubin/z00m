...
YUI().use('node', 'event', 'io', 'jsonp', 'yql', function(Y) {
    ...
    var _uri = "http://lotfellow.corp.yahoo.com",
        _api = _uri + "/v1",
        _cat = {
            301:  'News',
            700:  'Deal',
    ...
    Instant = {
        ...
        init: function () {
            ...
            if (WoeMap) {
                ...
                Y.jsonp(_api + "/woemap?path=/ca/san-francisco&format=json&callback={callback}",
        ...
        findPlace: function (e) {
            var latLng = {
                lat: e.latLng.va || e.latLng.xa,
                lng: e.latLng.wa || e.latLng.za
            };
        ...
        findZone: function (e, response) {
            ...
            var query = place.neighborhood;
            if (!query) {
                query = place.street + ", " + place.city + ", " + place.statecode;
            }
            
            var yquery = "select * from geo.places where text=\"" + query + "\" and placetype = 22";
